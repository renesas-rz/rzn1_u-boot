			...
			u32 ddr_start_addr, u32 ddr_size)
{
	...
	/* AXI port protection => disable */
	ddrc_writeb(0x00, base8 + DDR_AXI_PORT_PROT_ENABLE_REG);

	/* Although we have disabled port protection above, set up everything
	 * else so that it should work if it was enabled.
	...
	for (axi = 0; axi < DDR_NR_AXI_PORTS; axi++) {
		...
		/* Clear all other address range entries */
