...
static const uint8_t rzn1_rpkg_public_key_hash[] = { CONFIG_SPL_RZN1_RPKG_HASH };
...
static void __noreturn stop(void)
{
	hang();
...
static int validate_load_range(
	struct loaded_data_t *info)
{
	int i,j;
	int valid_addr = 0;
	/*
 ...
 */
static struct spkg_file_header *
locate_valid_spkg_header(
	struct spkg_file_header *h)
{
	int i;
...
static int parse_spkg_header(struct spkg_file_header *h, struct loaded_data_t *info)
{
...
#warning LIBDFDT is missing from SPL!
	...
	jump_to_image_linux((void *)dtb->load_addr);
	/* Should not return! */
}
	...
	for (; retries_count;
		...
		if (!verify && table->magic != PKGT_MAGIC) {
			if (be32_to_cpu(table->magic) == IH_MAGIC) {
				...
				case PKGT_SRC_QSPI:
#if defined(CONFIG_SPL_SPI_LOAD)
					spl_spi_load_one_uimage(flash, table_offset);
#endif
					break;
				case PKGT_SRC_NAND:
#if defined(CONFIG_SPL_NAND_LOAD)
					...
					spl_nand_load_one_uimage(table_offset);
#endif
					break;
				}
