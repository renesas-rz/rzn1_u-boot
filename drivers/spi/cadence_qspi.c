	...
	unsigned int delay)
{
	...
	/* sample on rising edge of ref clk */
	reg |= (1 << CQSPI_REG_RD_DATA_CAPTURE_EDGE_LSB);
	...
	unsigned int rxlen, u8 *rxbuf)
{
	...
	if (!cmdlen || rxlen > CQSPI_STIG_DATA_LEN_MAX || rxbuf == NULL) {
		printf("QSPI: Invalid input arguments cmdlen %u rxlen %u\n",
		       cmdlen, rxlen);
		return -EINVAL;
	}
	...
	const u8 *cmdbuf, unsigned int txlen, const u8 *txbuf)
{
	...
	debug("%s cmdlen:%d cmdbuf %p txlen:%d\n", __func__,
			cmdlen, cmdbuf, txlen);

	if (!cmdlen || cmdlen > 5 || txlen > 8 || cmdbuf == NULL) {
		printf("QSPI: Invalid input arguments cmdlen %u txlen %u\n",
		       cmdlen, txlen);
		return -EINVAL;
	}
...
int spi_cs_is_valid(unsigned int bus, unsigned int cs)
{
	...
	if ((cs < max_cs) && (bus < max_bus)) {
		return 1;
	}
	printf("QSPI: Invalid bus or cs. Bus %u cs %u\n", bus, cs);
	return 0;
}
...
void spi_slave_register_flash(struct spi_slave *slave, struct spi_flash *flash)
{
	...
	debug(" CMD rd %2x wr %02x addr:%d\n", flash->read_cmd,
		flash->write_cmd, qslave->slave.addressing_bytes);
	...
	if (qslave->slave.addressing_bytes < 3)
		qslave->slave.addressing_bytes = 3;
	...
	writel(wr_reg, &qslave->regbase->devwr);
		...
		unsigned int max_hz, unsigned int mode)
{
	...
	if (!qslave) {
		printf("QSPI: Can't allocate. Bus %u cs %u\n", bus, cs);
		return NULL;
	}
	...
	qslave->slave.op_mode_tx = SPI_OPM_TX_QPP;
	qslave->mode = mode;
		...
		void *data_in, unsigned long flags)
{
	...
	if (flags & SPI_XFER_BEGIN) {
		...
		debug("%s SPI_XFER_BEGIN cmd_len: %d\n", __func__, qslave->cmd_len);
	...
	if ((flags & SPI_XFER_END) || (flags == 0)) {
		if (data_in && data_bytes) {
			...
			debug("%s SPI_XFER_END cmd_len: %d %p\n",
				__func__, qslave->cmd_len, data_out);
